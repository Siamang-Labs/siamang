...
| Property | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `min_answers` | `int` | `1` | The minimum number of choices once the question is answered: the runtime refuses Next below it ("Select at least N more"), except for an `exclusive` answer, which stands alone. Whether the question may be left empty is `required`'s business. |
...
| `mode` | `str` | `"array"` | Determines how the data is structured. Allowed values: `"array"` (stores selected codes as a list in a single column) or `"wide"` (one variable per option, stored under its own name: `1` when chosen, `0` when the question is answered and the option is not; nothing while unanswered, nor for an option its own `show_if` / `hide_if` hid). In wide mode, `choices` of the same length as `var` pair choice *i* with variable *i*, so `exclusive` names choice codes; without them each variable is an option and `exclusive` names variable names. |
...
* **`simulate(n: int = 100, seed: int = 42) -> SurveyData`**:
  Generates `n` synthetic, logically valid responses using Monte Carlo simulation, which is useful for testing analytical pipelines before data collection [1]. Each respondent walks the pages as the runtime would move them: page, block, question and answer-option `show_if` / `hide_if`, `skip_to`, `next_if`, `default_next` and terminal pages. It runs the questionnaire's scripts too — the arm of every `Script.assign_condition` (a column, and a nominal codebook entry labeled with the arms) and the page order `Script.randomize_pages` deals: it is `siamang.local_simulator.simulate_survey(self, n=n, seed=seed)`. Quotas live in the compiler options, not on the questionnaire; `simulate_survey(survey, n, seed, quotas=…)` adds them — a balanced assignment balanced against its cells, and cells that close once their completes reach the limit. A questionnaire without those scripts gets the same frame as the page walk alone.
* **`preview() -> str`**:
